...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	squareSpeed := math.Pow(meanSpeedInMsec, 2)
	heightInM := height / cmInM
	calories := (walkingCaloriesWeightMultiplier*weight + (squareSpeed/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return calories
}
